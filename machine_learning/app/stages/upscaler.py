...
class RealESRGAN:
    def __init__(self, scale=2):
        ...
        Args:
            ...
            scale (int): Scaling factor. Must be one of [2, 4, 8].
        """
    ...
    def load_weights(self, model_path):
        """
        Loads the model weights from a local file.

        Args:
            model_path (str): Path to the model weights file.
        """
        if not os.path.exists(model_path):
            raise FileNotFoundError(f"Model weights not found at {model_path}. Please ensure the weights file is present.")
    ...
    def upscale(self, lr_image, batch_size=4, patches_size=192,
                padding=24, pad_size=15):
        """
        Performs super-resolution on the input image.

        Args:
            lr_image (PIL.Image): Low-resolution input image.
            batch_size (int): Number of patches to process at once.
            patches_size (int): Size of each patch.
            padding (int): Padding size for overlapping patches.
            pad_size (int): Padding size for the entire image.

        Returns:
            PIL.Image: Super-resolved output image.
        """
